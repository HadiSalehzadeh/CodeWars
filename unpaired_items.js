...
function hotSingles(arr1, arr2) {
  ...
  for (i = 0; i < arr1.length; i++) {
    if (!arr2.includes(arr1[i]) && !newArr.includes(arr1[i])) newArr.push(arr1[i])
  }
  for (i = 0; i < arr2.length; i++) {
    if (!arr1.includes(arr2[i]) && !newArr.includes(arr2[i])) newArr.push(arr2[i])
  }
  return newArr
}

console.log(hotSingles([1, 2, 3, 3], []))