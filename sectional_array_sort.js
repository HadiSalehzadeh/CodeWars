...
function sectSort(arr, start, length = 0) {
  let end = length > 0 ? start + length : arr.length;
  let section = arr.slice(start, end).sort((a, b) => a - b);
  arr.splice(start, length || arr.length - start, ...section);
  return arr;
}
